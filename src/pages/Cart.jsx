    ...
    <div className='w-full bg-gray-100 p-4'> 
      {
        ...
        <div>
          {
            products.map((item)=>(
              <div key={item.id} className='w-full border-b-[1px] border-b-[1px] border-b-gray-300 p-4 flex items-center gap-6'>
            ...
            <div onClick={()=>dispatch(resetCart())}>
              <button className='px-10 py-2 bg-red-500 hover:bg-red-600 accent-indigo-500 text-white rounded-lg font-titleFont font-semibold text-lg tracking-wide'>Clear Cart</button>
            </div>
        </div>
        <div className='w-full h-52 bg-white col-span-1 flex flex-col justify-center items-center p-4'>
            ...
            <div>
          <p className='font-semibold px-10 py-1 gap-2 flex items-center justify-between'>Total: <span className='text-lg font-bold'>${totalPrice}</span></p>
        </div>
                   <button className='w-full py-1.5 text-sm font-normal rounded-sm 
            bg-gradient-to-t from-[#f7dfa5] to-[#f0c14b] hover:bg-gradient-to-b border
          border-zinc-400 active:border-yellow-800 active:shadow-amazonInput'>Proceed to Pay</button>
        </div>
